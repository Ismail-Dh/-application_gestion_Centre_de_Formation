<?php 
include('../includes/config.php');

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Vérification des champs et nettoyage des données 
    $ID = mysqli_real_escape_string($db_conn, $_POST['ID_prof']);
    $nom = mysqli_real_escape_string($db_conn, $_POST['nom_prof']);
    $prenom = mysqli_real_escape_string($db_conn, $_POST['prenom_prof']);
    $telephone = mysqli_real_escape_string($db_conn, $_POST['telephone']);
    $adresse = mysqli_real_escape_string($db_conn, $_POST['adresse']);
    $idmatiere = mysqli_real_escape_string($db_conn, $_POST['ID_matiere']);

    // Requête SQL pour insérer les données dans la table des professeurs
    $insert_query = "INSERT INTO professeurs (ID_prof, nom_prof, prenom_prof, telephone, adresse, ID_matiere) VALUES ('$ID','$nom', '$prenom', '$telephone','$adresse','$idmatiere')";
    
    if (mysqli_query($db_conn, $insert_query)) {
        // Succès de l'insertion, redirection vers une page de succès par exemple
        header("Location: prof.php");
        exit();
    } else {
        // En cas d'erreur lors de l'insertion, gestion de l'erreur
        echo "Erreur lors de l'ajout de professeur : " . mysqli_error($db_conn);
    }
} else {
    // Si la méthode de requête n'est pas POST, redirection vers une page d'erreur par exemple
    header("Location: erreur.php");
    exit();
}
?>
